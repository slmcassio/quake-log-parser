...
public class Printer {
     ...
     *            The given {@link Game} {@link List}
     */
    private void print(final List<Game> games) {
    ...
    private void printPlayers(final Game game) {
        final String players = game.getPlayers().stream().map(Player::getName).map(this::appendDoubleQuotes).collect(joining(", "));
    ...
    /**
     * Prints the given {@link Game} suicides.
     * 
     * @param game
     *            The given {@link Game}.
     */
    private void printSuicides(final Game game) {
        if (game.getSuicides().isEmpty()) {
            return;
        }

        System.out.println(TAB + "Suicides: {");
        final Iterator<Suicides> suicides = game.getSuicides().iterator();

        while (suicides.hasNext()) {
            final Suicides suicide = suicides.next();
            final Player player = suicide.getPlayer();

            System.out.print(TAB + TAB + "\"" + player.getName() + "\": " + suicide.getCount());
