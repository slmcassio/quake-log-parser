...
import br.com.cassio.quakelog.parser.components.analyzers.SuicideAnalyzer;
import br.com.cassio.quakelog.parser.components.analyzers.WorldKillerAnalyzer;
...
public class SingleGameParser {
    ...
    private Game parseSingleGameLog(final SingleGameLog singleGameLog) {
        ...
        final Map<String, Suicides> suicideByName = new HashMap<>();

        final List<String> logLines = singleGameLog.getLogLines();
        parseLines(logLines, playerByUserId, playerByName, killByName, deathByName, suicideByName);
        ...
        final List<Suicides> suicides = new ArrayList<>(suicideByName.values());

        return new Game(name, players, kills, deaths, suicides);
    }
     ...
     */
    private void parseLines(final List<String> logLines, final Map<String, Player> playerByUserId, final Map<String, Player> playerByName,
        ...
        for (final String logLine : logLines) {
            parseLine(logLine, playerByUserId, playerByName, killByName, deathByName, suicideByName);
        }
    }
     ...
     *            A {@link Map} with the {@link Suicides} for a given name.
     */
    private void parseLine(final String logLine, final Map<String, Player> playerByUserId, final Map<String, Player> playerByName,
            final Map<String, Kills> killByName, final Map<String, Deaths> deathByName, final Map<String, Suicides> suicideByName) {
        ...
        if (killInfoMatcher.matches()) {
            ...
            parseKillInfoLine(killInfo, playerByUserId, killByName, deathByName, suicideByName);
     ...
     *            A {@link Map} with the {@link Suicides} for a given name.
     */
    private void parseKillInfoLine(final String killInfo, final Map<String, Player> playerByUserId, final Map<String, Kills> killByName,
            final Map<String, Deaths> deathByName, final Map<String, Suicides> suicideByName) {
        ...
        prepareAnalyzerChain().resolve(killerId, killedId, deathTypeId, playerByUserId, killByName, deathByName, suicideByName);
    }
     ...
     * 
     * <p>
     * - {@link WorldKillerAnalyzer}<br/>
     * - {@link SuicideAnalyzer}<br/>
    ...
    private AnalyzerChain prepareAnalyzerChain() {
        ...
        final AnalyzerChain suicideAnalyzer = new SuicideAnalyzer(Optional.of(deathAnalyzer));
        final AnalyzerChain worldKillerAnalyzer = new WorldKillerAnalyzer(Optional.of(suicideAnalyzer));

        return worldKillerAnalyzer;
    }
     ...
     */
    private void parseUserInfoLine(final String userInfo, final Map<String, Player> playerByUserId, final Map<String, Player> playerByName) {
